...
export default function Menu() {
  ...
  return (
    <div id="menu-container" className="fixed top-0 w-full h-20 z-50 bg-rose-1000">
      {/* Mobile Menu Button */}
      ...
      <menu id="menu" className="fixed hidden md:block top-0 left-0 shadow-sm z-40">
        <div className="top-44 md:top-28 fixed w-full justify-center z-40 bg-black/50 h-full md:h-12 items-center border-t-[1px] border-solid border-brown-1000">
          <div className='z-50 flex flex-col items-center justify-start h-full w-[70%] gap-2 bg-white p-2 md:w-full md:flex-row md:justify-center md:align-middle md:h-12'>
            {categories.map((category) => (
              <Link key={category.name} href={category.path}
                className={`p-3 no-underline border-solid border-b-[1px] md:w-fit border-brown-1000 w-full text-left text-black-1000 cursor-pointer
                   ${isActive(category.path) ? 'border-b-2 border-brown-1000 font-bold' : 'border-none'}`}
      ...
      {/* Header */}
      <div className="fixed top-8 w-full h-20 flex flex-row justify-between md:justify-between bg-rose-1000 items-center z-50">
